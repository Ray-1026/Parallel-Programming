...
static forceinline uint64_t _wymix(uint64_t A, uint64_t B) noexcept
{
    _wymum(&A, &B);
    return A ^ B;
}
...
// read functions
#if WYHASH_LITTLE_ENDIAN
static forceinline uint64_t _wyr8(const void *p) noexcept
{
    uint64_t v;
    memcpy(&v, p, 8);
    return v;
}
static forceinline uint64_t _wyr4(const void *p) noexcept
{
    uint32_t v;
    memcpy(&v, p, 4);
    return v;
...
#endif
static forceinline uint64_t _wyr3(const uint8_t *p, size_t k) noexcept
{
    return (((uint64_t)p[0]) << 16) | (((uint64_t)p[k >> 1]) << 8) | p[k - 1];
}

// A useful 64bit-64bit mix function to produce deterministic pseudo random numbers that can pass BigCrush and PractRand
static forceinline uint64_t wyhash64(uint64_t A, uint64_t B) noexcept
{
...
static forceinline uint64_t wyrand(uint64_t *seed) noexcept
{
...
static forceinline uint64_t wy2u0k(uint64_t r, uint64_t k) noexcept
{
    _wymum(&r, &k);
    return k;
}
...
struct rand {
    ...
    forceinline uint64_t operator()() noexcept { return internal::wyrand(&state); }
    ////////////////////////////////////////////////////////////////////////////////////
    ...
    /// <param name="max_value">The maximum value (exclusive)</param>
    /// <returns>The random value</returns>
    forceinline uint64_t uniform_dist(uint64_t max_value) noexcept { return internal::wy2u0k(operator()(), max_value); }
#endif
    ...
    {
        size_t sizeOf64 =
            (size * sizeof(T) + sizeof(uint64_t) - 1) / sizeof(uint64_t); // The number of 64-bits numbers to generate
...
struct hash_imp {
    ...
    /// </summary>
    hash_imp() noexcept
        : secret{0xa0761d6478bd642full, 0xe7037ed1a0b428dbull, 0x8ebc6af09c88c6e3ull, 0x589965cc75374cc3ull}
    ...
    hash_imp(const uint64_t psecret[4]) noexcept { memcpy(secret, psecret, sizeof(secret)); }
    ...
    {
        // code taken from 'wyhash.h::wyhash(const void* key, size_t len, uint64_t seed, const uint64_t * secret)' with
        // seed=0
    ...
    /// <returns>A 64-bits hash</returns>
    forceinline uint64_t wyhash(uint64_t number) const noexcept { return internal::wyhash64(number, secret[0]); }
};
...
template <class STRING_TYPE> struct hash_string_base : private hash_imp {
    ...
    forceinline uint64_t operator()(const STRING_TYPE &elem) const noexcept
    {
        return hash_imp::wyhash(reinterpret_cast<const uint8_t *>(elem.data()),
                                sizeof(typename STRING_TYPE::value_type) * elem.size());
    }
};
}; // namespace internal
...
template <class T> struct hash : private internal::hash_imp {
    ...
    forceinline uint64_t operator()(const T &elem) const noexcept
    {
        ...
        if (sizeof(T) == 4)
            return hash_imp::wyhash(internal::_wyr4(&elem));
        if (sizeof(T) == 8)
            return hash_imp::wyhash(internal::_wyr8(&elem));
        if (sizeof(T) == 16)
            return internal::wyhash64(internal::_wyr8(&elem),
                                      internal::_wyr8(reinterpret_cast<const uint8_t *>(&elem) + 8));
