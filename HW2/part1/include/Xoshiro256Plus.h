...
template <SIMDInstructionSet SIMD> class Xoshiro256Plus {
  public:
    class FourIntegerValues {
      ...
      private:
        ...
        FourIntegerValues(FourIntegerValues &&value_to_copy) : result_packed_(std::move(value_to_copy.result_packed_))
        {
        }
    ...
    enum class JumpOnCopy : int32_t { None = 0, Short, Long };
    ...
    Xoshiro256Plus(const uint64_t seed)
    {
        ...
        static_assert(SIMD == SIMDInstructionSet::NONE, "Cannot have an AVX2 RNG if AVX2 extensions are not available");
#endif
    ...
    Xoshiro256Plus(const Xoshiro256Plus<SIMD> &rng_to_copy, JumpOnCopy jump_dist = JumpOnCopy::Short)
        : serial_state_(rng_to_copy.serial_state_), serial_next4_state_(rng_to_copy.serial_next4_state_),
    ...
    uint64_t next(uint32_t lower_bound, uint32_t upper_bound)
    {
        ...
        return (((uint64_t)((uint32_t)next()) * (uint64_t)(upper_bound - lower_bound)) >> 32) + (uint64_t)lower_bound;
    }
    ...
    FourIntegerValues next4(uint32_t lower_bound, uint32_t upper_bound)
    {
        ...
        if constexpr (SIMD >= SIMDInstructionSet::AVX2) {
            ...
            four_ints.result_packed_[0] = (((uint64_t)((uint32_t)four_ints[0]) * range) >> 32) + (uint64_t)lower_bound;
            four_ints.result_packed_[1] = (((uint64_t)((uint32_t)four_ints[1]) * range) >> 32) + (uint64_t)lower_bound;
            four_ints.result_packed_[2] = (((uint64_t)((uint32_t)four_ints[2]) * range) >> 32) + (uint64_t)lower_bound;
            four_ints.result_packed_[3] = (((uint64_t)((uint32_t)four_ints[3]) * range) >> 32) + (uint64_t)lower_bound;
    ...
    FourDoubleValues dnext4(double lower_bound, double upper_bound)
    {
        ...
        if constexpr (SIMD >= SIMDInstructionSet::AVX2) {
            return _mm256_add_pd(_mm256_mul_pd(dnext4(), _mm256_set1_pd(upper_bound - lower_bound)),
                                 _mm256_set1_pd(lower_bound));
        }
        else {
            auto result = dnext4();
